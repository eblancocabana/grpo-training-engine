...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Plot tokens/sec history for optimizer attempts"
    )
    parser.add_argument(
        "--ledger",
        type=Path,
    ...
    svg_path = args.output_dir / "tokens_per_sec.svg"
    md_path = args.output_dir / "tokens_per_sec.md"
...
def write_svg_plot(rows: list[ExperimentLedgerRecord], svg_path: Path) -> None:
    ...
    frontier_values = [
        row.incumbent_tokens_per_sec_after_decision or 0.0 for row in rows
    ]
    candidate_values = [row.candidate_snapshot.tokens_per_sec or 0.0 for row in rows]
...
<text x=\"{margin}\" y=\"24\" font-family=\"sans-serif\" font-size=\"18\">Generation tokens/sec over attempts</text>
...
<text x=\"{width - 220}\" y=\"{margin}\" font-family=\"sans-serif\" font-size=\"12\" fill=\"#1f77b4\">Candidate tokens/sec</text>
...
def write_report(
    ...
    latest_frontier = latest.incumbent_tokens_per_sec_after_decision if latest else None
    md_path.write_text(
        "# Tokens Per Second History\n\n"
        f"- Total attempts: {total_attempts}\n"
        f"- Kept: {kept}\n"
        f"- Discarded: {discarded}\n"
        f"- Latest frontier tokens/sec: {latest_frontier}\n"
        f"- Plot: `{plot_path}`\n",
        encoding="utf-8",
    )
