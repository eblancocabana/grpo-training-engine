...
class MemoryManager:
    def __init__(
        ...
        checkpointing_layer_name_patterns: Optional[List[str]] = None,
        checkpointing_layer_types: Optional[List[str]] = None,
        ...
        self.checkpointing_update_interval_steps = checkpointing_update_interval_steps
        self._checkpointing_enabled = False
        self._last_checkpointing_update_step = -1
        self.step_count = 0
    ...
    def enable_checkpointing(self, model: torch.nn.Module):
        should_enable = (
            self.enable_gradient_checkpointing and self.checkpointing_strategy != "none"
        )
        ...
        if should_enable:
            ...
            self._checkpointing_enabled = False
            logger.info(
                "Gradient checkpointing disabled (%s).",
    ...
    def maybe_update_checkpointing(self, model: torch.nn.Module, step: int):
        ...
        if self.checkpointing_strategy != "vram_auto":
            return

        if step == self._last_checkpointing_update_step:
            return

        if step % max(1, self.checkpointing_update_interval_steps) != 0:
            return

        self._last_checkpointing_update_step = step

        available_memory_gb = self.get_available_memory_gb()
        stats = self.get_memory_stats()
        usage_fraction = stats.get("usage_fraction", 0.0)

        if self._checkpointing_enabled:
            if usage_fraction <= self.checkpointing_vram_disable_threshold:
                logger.info(
                    "Gradient checkpointing remains enabled (vram_auto, free=%.2fGB).",
                    available_memory_gb,
                )
        elif usage_fraction >= self.checkpointing_vram_enable_threshold:
