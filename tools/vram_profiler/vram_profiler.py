...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="vram_snapshot.json", help="Output JSON file"
    )
    parser.add_argument(
        "--context",
        choices=["all", "state", "none"],
        default="all",
        help="Stack trace level",
    )
    parser.add_argument(
        "--max-entries", type=int, default=2000000, help="Max history entries"
    )
    parser.add_argument(
        "--serve", action="store_true", help="Serve GUI and API after capture"
    )
    args = parser.parse_args()

    start_profiling(args.context, args.max_entries)

    compress_snapshot_module = None
    try:
        ...
        from tools.vram_profiler import compress_snapshot

        compress_snapshot_module = compress_snapshot
        ...
        def limited_train_epoch(dataloader, epoch):
            ...
            mock_self = trainer
            if mock_self.model is not None:
        ...
        if os.path.exists(args.output):
            ...
            if file_size_mb > AUTO_COMPRESS_THRESHOLD_MB:
                ...
                try:
                    if compress_snapshot_module is None:
                        raise RuntimeError("compress_snapshot module missing")
                    compress_snapshot_module.compress_snapshot(
                        args.output, compressed_path
                    )
                    ...
                    print(
                        f"[Profiler] Compression complete: {new_size:.2f} MB. Original deleted."
                    )
                except Exception as e:
        ...
        if args.serve:
            try:
                ...
                os.environ["VRAM_PROFILER_SNAPSHOT_DIR"] = os.path.dirname(
                    os.path.abspath(args.output)
                )
                start_server(port=8550)
                print("[Profiler] Server running at http://localhost:8550")
                try:
                    while True:
                        time.sleep(1)
                except KeyboardInterrupt:
                    print("[Profiler] Server stopped.")
            except Exception as e:
                print(f"[Profiler] Server failed: {e}")

        if torch.cuda.is_available():
            stats = torch.cuda.memory_stats()
            print(
                f"Final VRAM: {stats.get('allocated_bytes.all.current', 0) / 1024**3:.2f}GB allocated"
            )
