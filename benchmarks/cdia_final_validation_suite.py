#!/usr/bin/env python3
"""
CDIA final-validation benchmark suite for the PFG.

This suite mirrors the runner/reporting flow from
`cdia_finalist_followup_suite.py`, but narrows the plan to a final-validation
set with explicit per-seed runs and a clearly separated exploratory ablation.

Methodology split:
- strict finalist comparison: 4 no-SENT runs kept apples-to-apples with the
  existing finalist-followup evidence
- exploratory difficulty-weighting ablation: 1 separate run that may enable
  SENT because its weighting signal is explicitly a SENT-rank difficulty proxy
"""
...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "benchmarks" / "output" / "cdia_final_validation"
DEFAULT_TRAINER_STEPS = 120
STRICT_FAMILY = "strict_finalist_comparison"
EXPLORATORY_FAMILY = "exploratory_difficulty_weighting"
...
class BenchmarkResult:
    """Runtime summary for a single CDIA final-validation config."""
...
class SuiteConfig:
    ...
    def train_args(
        ...
        if resolved["do_sample"] is False:
            args.append("--greedy")
        if resolved["difficulty_weighting_mode"] != "off":
            args.extend(
                [
                    "--difficulty-weighting-mode",
                    str(resolved["difficulty_weighting_mode"]),
                    "--difficulty-weighting-min-weight",
                    str(resolved["difficulty_weighting_min_weight"]),
                    "--difficulty-weighting-max-weight",
                    str(resolved["difficulty_weighting_max_weight"]),
                ]
            )
        if use_wandb:
            args.extend(
...
def build_suite() -> list[SuiteConfig]:
    return [
        SuiteConfig(
            ...
            description="Strict finalist comparison: no length penalty, seed 1, SENT off",
            overrides={
            ...
            notes=[
                "Strict finalist comparison run.",
                "SENT stays disabled to match the current finalist-followup evidence regime.",
            ],
        ),
        SuiteConfig(
            name="length_penalty_0_seed2",
            family=STRICT_FAMILY,
            description="Strict finalist comparison: no length penalty, seed 2, SENT off",
            overrides={
                "length_penalty_coef": 0.0,
                "seed": 2,
                "sent_enabled": False,
        ...
        SuiteConfig(
            ...
            family=STRICT_FAMILY,
            description="Strict finalist comparison: len_penalty=0.0005, mask_truncated=False, seed 1, SENT off",
            overrides={
            ...
            notes=[
                "Strict finalist comparison run.",
                "Keeps the no-SENT finalist-followup regime while testing the truncation-mask interaction finalist.",
            ],
        ),
        SuiteConfig(
            name="length_penalty_0005_mask_truncated_off_seed2",
            family=STRICT_FAMILY,
            description="Strict finalist comparison: len_penalty=0.0005, mask_truncated=False, seed 2, SENT off",
            overrides={
                "length_penalty_coef": 0.0005,
                "mask_truncated_completions": False,
                "seed": 2,
        ...
        SuiteConfig(
            name="length_penalty_0_difficulty_weighting_seed1",
            family=EXPLORATORY_FAMILY,
            description="Exploratory ablation: no length penalty, seed 1, SENT-rank difficulty proxy enabled",
            overrides={
                "length_penalty_coef": 0.0,
                "seed": 1,
                "sent_enabled": True,
                "difficulty_weighting_mode": "sent_rank_linear",
                "difficulty_weighting_min_weight": 1.0,
                "difficulty_weighting_max_weight": 1.2,
            },
            notes=[
                "Exploratory ablation only; do not use it as part of the strict winner comparison.",
                "Enables SENT only because the weighting signal is explicitly a SENT-rank difficulty proxy.",
            ],
        ),
    ]
...
) -> tuple[list[SuiteConfig], list[SuiteConfig]]:
    strict = [cfg for cfg in configs if cfg.family == STRICT_FAMILY]
    exploratory = [cfg for cfg in configs if cfg.family == EXPLORATORY_FAMILY]
    return strict, exploratory
    ...
    for cfg in configs:
        ...
        if cfg.runnable:
            print(
                ...
                f"difficulty_weighting={resolved['difficulty_weighting_mode']} "
...
def print_summary(configs: list[SuiteConfig], trainer_steps: int) -> None:
    strict, exploratory = _split_configs(configs)
    print("CDIA final validation suite")
    print(f"Trainer steps per config: {trainer_steps}")
    print(f"Configs in suite: {len(configs)}")
    print(f"Strict finalist comparison runs: {len(strict)}")
    print(f"Exploratory ablation runs: {len(exploratory)}")
    print(f"Baseline effective samples/update: {BASELINE.effective_samples_per_update}")
    print(
        "Methodology: keep the 4 strict finalist runs in the same no-SENT regime as "
        "the finalist-followup evidence, and keep the difficulty-weighting run separate."
    )
    print(
        "Difficulty weighting remains off by default and is enabled only for the "
        "single exploratory SENT-rank proxy run."
    )
    print()
    _print_config_section("Strict Finalist Comparison", strict)
    _print_config_section("Exploratory Ablation", exploratory)
...
def write_plan(configs: list[SuiteConfig], output_path: Path, trainer_steps: int) -> None:
    ...
    payload = {
        ...
        "strict_finalist_run_names": [cfg.name for cfg in strict],
        "exploratory_run_names": [cfg.name for cfg in exploratory],
        "configs": [
            {
                "name": cfg.name,
                "family": cfg.family,
...
class CdiaBenchmarkSuite:
    """CDIA final-validation suite with the finalist-followup runner flow."""
    ...
    def __init__(
        ...
        self.suite_name = "CDIA Final Validation Suite"
        self.report_title = "CDIA Final Validation Report"
    ...
    def _append_summary_table(
        self, lines: list[str], title: str, results: list[BenchmarkResult]
    ) -> None:
        lines.extend(
            [
                f"## {title}",
        ...
        for result in results:
            cfg = self._config_for_result(result)
            resolved = cfg.resolved() if cfg is not None else {}
            ...
            lines.append(
                f"| {result.config_name} | {resolved.get('seed', 'N/A')} | "
                f"{resolved.get('sent_enabled', 'N/A')} | "
                f"{resolved.get('difficulty_weighting_mode', 'N/A')} | {status} | "
                f"{result.steps_completed} | {opt_steps} | {tok_s} | {reward_final} | "
                f"{reward_peak} | {reward_drop} | {final_resp_len} | {peak_resp_len} | "
                f"{actual_trunc_avg} | {masked_out_avg} |"
            )
        lines.append("")
    ...
    def _generate_report(self) -> None:
        ...
        strict_results = [
            ...
            if (cfg := self._config_for_result(result)) is not None
            and cfg.family == STRICT_FAMILY
        ]
        exploratory_results = [
            result
            for result in self.results
            if (cfg := self._config_for_result(result)) is not None
            and cfg.family == EXPLORATORY_FAMILY
        ]
        ...
        self._append_summary_table(lines, "Strict Finalist Comparison", strict_results)
        ...
        for result in self.results:
            ...
            if cfg is not None:
                lines.append(f"- Comparison group: {cfg.family}")
            if resolved:
                lines.append(f"- Seed: {resolved['seed']}")
                lines.append(f"- SENT enabled: {resolved['sent_enabled']}")
                lines.append(
                    f"- Difficulty weighting mode: {resolved['difficulty_weighting_mode']}"
                )
                if resolved["difficulty_weighting_mode"] != "off":
                    lines.append(
                        "- Difficulty weighting range: "
                        f"[{resolved['difficulty_weighting_min_weight']:.3f}, "
                        f"{resolved['difficulty_weighting_max_weight']:.3f}]"
                    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="CDIA final validation planner/runner"
    )
    parser.add_argument(
        "--steps",
        type=int,
        default=DEFAULT_TRAINER_STEPS,
        help="Trainer steps per config for the final-validation suite. Fixed default: 120.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    parser.add_argument(
        "--list",
        action="store_true",
        help="Print the final-validation suite summary and exit without running.",
    )
    parser.add_argument(
        "--filter-configs",
        type=str,
...
def main() -> int:
    ...
    default_plan_name = (
        f"{normalized_prefix}_final_validation_suite_plan.json"
        if normalized_prefix
        else "cdia_final_validation_suite_plan.json"
    )
